$(document).ready(function () {
  ...
  var lang = null;
  /* globale dato che la uso anche altrove */
  var load = false;
  //gestisce i caricamenti ed evita sovrapposizioni. Dichiarata qui che è il foglio che viene incluso di default ovunque e per primo
  ...
  $('#footersearch').on('submit', function (e) {
    ...
    var num = 10;
    //TODO: numero di posts, parametro?
    if (qs === '') {
      return false;
    }
  ...
  plist.on('click', '.more', function () {
    var me = $(this), par = me.parent(), jenk = par.parent().children('div').eq(0);
  ...
  $('#nerdzselect').on('click', function () {
    if (!$(this).hasClass('icon_collapse'))
      $(this).attr('class', 'icon_collapse');
    else
      $(this).attr('class', 'icon_expand');
