...
$(document).ready(function () {
  ...
  $('#title_left').click(function () {
    if (moving)
      return;
    moving = 1;
    $('#right_col').removeClass('shown').animate({ left: '100%' }, 500);
    if (!$('#left_col').hasClass('shown'))
      $('#left_col').css('left', '-70%').show().animate({ left: '0%' }, 500, function () {
        $(this).addClass('shown');
        moving = 0;
      });
    else
      $('#left_col').animate({ left: '-70%' }, 500, function () {
  ...
  $('#title_right').click(function () {
    if (moving)
      return;
    moving = 1;
    $('#left_col').removeClass('shown').animate({ left: '-70%' }, 500);
    if (!$('#right_col').hasClass('shown'))
      $('#right_col').css('left', '100%').show().animate({ left: '30%' }, 500, function () {
        $(this).addClass('shown');
        moving = 0;
      });
    else
      $('#right_col').animate({ left: '100%' }, 500, function () {
        $(this).removeClass('shown').hide();
        moving = 0;
      });
    return false;
  ...
  plist.on('click', '.delcomment', function (e) {
    ...
    popup.on('click', '#delCommOk' + hcid, function () {
      N.json[plist.data('type')].delComment({ hcid: hcid }, function (j) {
        if (j.status === 'ok') {
    ...
    popup.attr('class', 'ui-content').css({
      ...
      color: '#111',
      padding: '15px'
    }).popup({ positionTo: 'window' }).popup('open');
  });
  ...
  plist.on('click', '.imglocked', function (e) {
    ...
    if ($(this).data('silent')) {
      ...
      N.json[plist.data('type')].reNotifyForThisPost({ hpid: $(this).data('hpid') }, function (d) {
        tog(d);
      });
    }
  });
  plist.on('click', '.imgunlocked', function (e) {
    ...
    if ($(this).data('silent')) {
      ...
      N.json[plist.data('type')].noNotifyForThisPost({ hpid: $(this).data('hpid') }, function (d) {
        tog(d);
      });
    }
  });
  plist.on('click', '.lurk', function (e) {
    ...
    e.preventDefault();
    N.json[plist.data('type')].lurkPost({ hpid: $(this).data('hpid') }, function (d) {
      tog(d);
    });
  });
  plist.on('click', '.unlurk', function (e) {
    ...
    e.preventDefault();
    N.json[plist.data('type')].unlurkPost({ hpid: $(this).data('hpid') }, function (d) {
      tog(d);
    });
  });
  plist.on('click', '.bookmark', function (e) {
    ...
    e.preventDefault();
    N.json[plist.data('type')].bookmarkPost({ hpid: $(this).data('hpid') }, function (d) {
      tog(d);
    });
  });
  plist.on('click', '.unbookmark', function (e) {
    ...
    e.preventDefault();
    N.json[plist.data('type')].unbookmarkPost({ hpid: $(this).data('hpid') }, function (d) {
      tog(d);
    });
  });
  ...
  plist.on('taphold', '.nerdz_message', function (e) {
    ...
    var mess = $(this).children('div').eq(0), more = mess.next();
    mess.hide();
    more.show();
    $(this).addClass('opted');
    setTimeout(function () {
  ...
  plist.on('click', '.vote', function () {
    var curr = $(this), cont = curr.parent(), tnum = cont.parent().children('.thumbs-counter'), func = 'thumbs', obj = { hpid: cont.data('refto') };
